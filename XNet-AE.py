...
SAVE_DIR = r"D:\桌面\XNet-AE\Hydraulic parameters"  
TIME_DIR = r"D:\桌面\XNet-AE\Time"  
PLOT_DIR = r"D:\桌面\XNet-AE\Training_plot"  
MOISTURE_DIR = r"D:\桌面\XNet-AE\Moisture Content"  
LOSS_DIR = r"D:\桌面\XNet-AE\Loss"  
...
# 6. Model parameters
MODEL_HIDDEN_LAYERS = [2, 30]  # Hidden layer configuration
...
class XNet(nn.Module):
    def __init__(self, hidden_layers):
        ...
        self.hidden = nn.Sequential(
            ComplexLinear(hidden_layers[0], hidden_layers[1])
        )
    ...
    def forward(self, x):
        y = self.hidden(x)
        y = self.sigmoid(y)
        y = self.theta_r + (self.theta_s - self.theta_r) * y
        return y
...
def loss_function(model, c_net, k_net, t, x):
    ...
    k_u = k_net(u)
    c_u = c_net(u)
    k_x = torch.autograd.grad(k_net(u), x, torch.ones_like(u), create_graph=True, retain_graph=True)[0]
    c_x = torch.autograd.grad(c_net(u), x, torch.ones_like(u), create_graph=True, retain_graph=True)[0]
...
def reconstruction_loss(c_net, k_net):
    ...
    return criterion(k_u, u_tensor) + criterion(c_u, u_tensor)
...
def main():
    ...
    try:
        ...
        model = XNet(MODEL_HIDDEN_LAYERS).to(device)
        ...
        for epoch in range(NUM_OUTER_LOOPS):
            ...
            with torch.no_grad():
                ...
                h_pred = c_net(u_tensor).cpu().numpy()
                k_pred = k_net(u_tensor).cpu().numpy()
